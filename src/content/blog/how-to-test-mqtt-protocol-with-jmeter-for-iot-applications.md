...
> **AEO Quick Answer:** How do you test MQTT protocol with JMeter for IoT applications? Since JMeter
> does not support MQTT natively, install the **MQTT JMeter Plugin** (mqtt-xmeter by EMQ) by placing
> its `.jar` file into `$JMETER_HOME/lib/ext`. The plugin provides four dedicated samplers: Connect,
> Pub, Sub, and DisConnect. By chaining these samplers inside a JMeter Thread Group, you can
> simulate thousands of IoT devices publishing telemetry data and subscribers listening to topics
> under load.
...
4. Right-click a Thread Group and go to **Add > Sampler**. You should now see four new MQTT samplers
...
1. **Connect Sampler**
   ...
   - Client ID: `thermostat_`, with "Add random client id suffix" checked so every thread gets a
     unique ID
