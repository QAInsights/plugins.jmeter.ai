...
async function renderVendorIndexOg(
  vendors: Map<string, VendorAggregate>,
  plugins: RawPlugin[],
): Promise<'written'> {
...
async function getBlogPosts(): Promise<
  Array<{ slug: string; ext: string; frontmatter: BlogFrontmatter; body: string }>
> {
  try {
...
async function renderBlogPostOg(post: {
  slug: string;
  ext: string;
...
async function renderCollectionOg(col: {
  id: string;
  name: string;
  emoji: string;
  tagline: string;
...
async function runWithConcurrency<T>(
  items: T[],
  fn: (item: T) => Promise<'written' | 'skipped' | 'error'>,
...
async function main() {
  ...
  console.log(
    `[og] plugins: ${pluginResults.written} written, ${pluginResults.skipped} skipped, ${pluginResults.failed} failed`,
  );
  ...
  console.log(
    `[og] vendors: ${vendorResults.written} written, ${vendorResults.skipped} skipped, ${vendorResults.failed} failed`,
  );
  ...
  console.log(
    `[og] blog posts: ${blogResults.written} written, ${blogResults.skipped} skipped, ${blogResults.failed} failed`,
  );
  ...
  const collectionResults = await runWithConcurrency(
    collections as any[],
    renderCollectionOg as any,
  );
  console.log(
    `[og] collections: ${collectionResults.written} written, ${collectionResults.skipped} skipped, ${collectionResults.failed} failed`,
  );
  ...
  const totalWritten =
    pluginResults.written +
    vendorResults.written +
    blogResults.written +
    collectionResults.written +
    collectionsIndexWritten +
    3; // +3 for vendor-index, blog-index, home singletons
  const totalSkipped =
    pluginResults.skipped +
    vendorResults.skipped +
    blogResults.skipped +
    collectionResults.skipped +
    collectionsIndexSkipped;
  const totalFailed =
    pluginResults.failed + vendorResults.failed + blogResults.failed + collectionResults.failed;
